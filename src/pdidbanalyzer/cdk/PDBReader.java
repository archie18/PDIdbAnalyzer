...
public class PDBReader extends DefaultChemObjectReader {
	...
	public IChemObject read(IChemObject oObj) throws CDKException {
		if (oObj instanceof IChemFile) {
			if (pdbFactory == null) {
				try {
                                        /** BUG-FIX: Corrected atom types XML file */
                                        //pdbFactory = AtomTypeFactory.getInstance("org/openscience/cdk/config/data/pdb_atomtypes.xml", 
					pdbFactory = AtomTypeFactory.getInstance("pdidbanalyzer/cdk/pdb_atomtypes.xml", 
	...
	private IChemFile readChemFile(IChemFile oFile) 	{
		...
		//boolean isProteinStructure = false;
                // TO-DO: Protein recognition is flawed. Assuming always true
                // as a work-around
		...
		try {
			do {
				...
				if (cRead != null) {	
					...
					} else if ("ATOM  ".equalsIgnoreCase(cCol) || "HETATM".equalsIgnoreCase(cCol)) {
						// read an atom record
...
//							oMonomer = oBP.getMonomer(cResidue.toString(), String.valueOf(chain));
                                                        // BUG FIX
							oMonomer = oBP.getMonomer(cResidue.toString(), strandName);
							if (oMonomer == null) {
...
//		}
		if (lineLength >= 79) {
            ...
            if (frag.length() > 0) {
                try {
                    // BUG-FIX
                    // Try reverse order, e.g. 1- => -1 which should be parsable as a number
                    oAtom.setCharge(Double.parseDouble(frag.substring(1, 2) + frag.substring(0, 1)));
                } catch (NumberFormatException ex) {
                    // Try original order, although against PDB specifications
